        ...
        try {
            if (isRecovery) {
                const { error } = await supabase.auth.resetPasswordForEmail(email, {
                    redirectTo: `${window.location.origin}/admin?reset_password=true`,
                });
                if (error) throw error;
                alert('¡Correo enviado! Revisa tu bandeja de entrada (y spam) para acceder a tu cuenta.');
                setIsRecovery(false);
            } else if (isRegister) {
        ...
        <div className="min-h-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
            <div className="sm:mx-auto sm:w-full sm:max-w-md">
                <h2 className="mt-6 text-center text-3xl font-extrabold text-gray-900">
                    {isRecovery ? 'Recuperar Cuenta' : (isRegister ? 'Crea tu cuenta' : 'Inicia Sesión')}
                </h2>
                {!isRecovery && (
                ...
                <div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
                    <form className="space-y-6" onSubmit={handleAuth}>
                        ...
                        <div>
                            ...
                            >
                                {loading ? 'Procesando...' : (isRecovery ? 'Enviar Link de Recuperación' : (isRegister ? 'Registrarse' : 'Entrar'))}
                            </button>
                        </div>
                    </form>
                </div>
